$(document).ready(function () {
  ...
  function init() {
    ...
    // ตรวจสอบการคลิกที่ลิงก์ใน Sidebar Menu
    $("#sidebar-menu a").on("click", function (e) {
      ...
      if (!$(this).hasClass("subdrop")) {
        ...
        // ปิดเมนูที่ถูกคลิกอีกครั้ง
        $(this).removeClass("subdrop");
        $(this).next("ul").slideUp(350);
      }
    });

    // เปิดเมนูที่มีคลาส active โดยอัตโนมัติ
    ...
    // ตรวจสอบ URL ปัจจุบันและเปิดเมนูซับที่ตรงกับ URL
    var currentPath = window.location.pathname.split("/").pop();
    $("#sidebar-menu a").each(function () {
      var href = $(this).attr("href");

      // ตรวจสอบว่าลิงก์ตรงกับ URL ปัจจุบัน
      if (currentPath === href) {
        $(this).addClass("active");

        // เปิดเมนูหลักและเพิ่มคลาส subdrop
        $(this).parents("li.submenu").children("a").addClass("subdrop active");
        $(this).parents("ul").slideDown();

        // เปิดเมนูหลักทั้งหมดที่เป็น parent
        $(this)
          .parents("ul")
          .each(function () {
            $(this).slideDown();
            $(this).prev("a").addClass("subdrop active");
          });
      }
    });
  }
  ...
  // ฟังก์ชันจัดการการเปิด-ปิด Mini Sidebar
  $(document).on("click", "#toggle_btn", function () {
    if ($("body").hasClass("mini-sidebar")) {
      ...
      // ตั้งค่าให้ Header แสดงสถานะ active
      setTimeout(function () {
        $("body").removeClass("mini-sidebar");
        $(".header-left").addClass("active");
      }, 100);
    } else {
      // ปิด Sidebar เป็น Mini
      $("body").addClass("mini-sidebar");
      $(this).removeClass("active");
      $(".subdrop + ul").slideUp();
      localStorage.setItem("sidebarState", "mini");

      // ตั้งค่าให้ Header ไม่แสดงสถานะ active
      setTimeout(function () {
        $("body").addClass("mini-sidebar");
  ...
  // ตรวจสอบสถานะ Sidebar จาก localStorage เมื่อโหลดหน้าใหม่
  $(document).ready(function () {
